	...
	protected function getList(): string {
		...
		foreach($results as $result) {
			...
			$data['products'][] = [
				...
				'edit' => $this->url->link('catalog/product.form', 'user_token='.$this->session->data['user_token'].'&product_id='.$result['product_id'].($result['variation_id'] ? '&variation_id='.$result['variation_id'] : '').$url),
				'variant' => (!$result['variation_id'] ? $this->url->link('catalog/product.form', 'user_token='.$this->session->data['user_token'].'&variation_id='.$result['product_id'].$url) : '')
			];
		}
...
$optionsave['option_id'] = 0;
 $optionsave['option_value'] = [];
	...
	public function form(): void {
		...
		if(isset($this->request->get['variation_id'])) {
			$data['variation_id'] = (int)$this->request->get['variation_id'];
		} elseif(!empty($product_info)) {
			$data['variation_id'] = $product_info['variation_id'];
		} else {
			$data['variation_id'] = 0;
		}
