...
class Marketing extends \Ventocart\System\Engine\Controller {
	...
	public function index(): void {
		$this->load->language('extension/ventocart/report/marketing');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/ventocart/report/marketing', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/ventocart/report/marketing.save', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/ventocart/report/marketing_form', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/ventocart/report/marketing')) {
	...
	public function report(): void {
		...
		$this->response->setOutput($this->load->view('extension/ventocart/report/marketing', $data));
	}
	...
	public function getReport(): string {
		...
		$this->load->model('extension/ventocart/report/marketing');

		$marketing_total = $this->model_extension_ventocart_report_marketing->getTotalMarketing($filter_data);

		$results = $this->model_extension_ventocart_report_marketing->getMarketing($filter_data);
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination'),
			'url'   => $this->url->link('extension/ventocart/report/marketing.report', 'user_token=' . $this->session->data['user_token'] . '&code=marketing' . $url . '&page={page}')
		]);
		...
		return $this->load->view('extension/ventocart/report/marketing_list', $data);
	}
}
