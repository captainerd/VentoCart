 ...
 */
class CustomerReward extends \Ventocart\System\Engine\Controller {
	/**
	 * @return void
	 */
	public function index(): void {
		$this->load->language('extension/ventocart/report/customer_reward');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/ventocart/report/customer_reward', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/ventocart/report/customer_reward.save', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/ventocart/report/customer_reward_form', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/ventocart/report/customer_reward')) {
	...
	public function report(): void {
		...
		$this->response->setOutput($this->load->view('extension/ventocart/report/customer_reward', $data));
	}
	...
	public function getReport(): string {
		...
		$customer_total = $this->model_extension_ventocart_report_customer->getTotalRewardPoints($filter_data);

		$results = $this->model_extension_ventocart_report_customer->getRewardPoints($filter_data);
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination'),
			'url'   => $this->url->link('extension/ventocart/report/customer_reward.report', 'user_token=' . $this->session->data['user_token'] . '&code=customer_reward' . $url . '&page={page}')
		]);
		...
		return $this->load->view('extension/ventocart/report/customer_reward_list', $data);
	}
}
