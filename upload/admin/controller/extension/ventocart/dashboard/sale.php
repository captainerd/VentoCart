 ...
 */
class Sale extends \Ventocart\System\Engine\Controller {
	/**
	 * @return void
	 */
	public function index(): void {
		$this->load->language('extension/ventocart/dashboard/sale');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/ventocart/dashboard/sale', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/ventocart/dashboard/sale.save', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/ventocart/dashboard/sale_form', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/ventocart/dashboard/sale')) {
	...
	public function dashboard(): string {
		...
		$today = $this->model_extension_ventocart_report_sale->getTotalSales(['filter_date_added' => date('Y-m-d', strtotime('-1 day'))]);

		$yesterday = $this->model_extension_ventocart_report_sale->getTotalSales(['filter_date_added' => date('Y-m-d', strtotime('-2 day'))]);
		...
		$sale_total = $this->model_extension_ventocart_report_sale->getTotalSales();
		...
		return $this->load->view('extension/ventocart/dashboard/sale_info', $data);
	}
}
