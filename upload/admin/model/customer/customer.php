 ...
 */
class Customer extends \Ventocart\System\Engine\Model
{
	/**
	 * @param array $data
	 *
	...
	public function getCustomers(array $data = []): array
	{
		...
		// Exclude customers with no valid email
	...
	{
		$this->db->query("INSERT INTO `" . DB_PREFIX . "address` SET `customer_id` = '" . (int) $customer_id . "', `firstname` = '" . $this->db->escape($data['firstname']) . "', `lastname` = '" . $this->db->escape($data['lastname']) . "', `company` = '" . $this->db->escape($data['company']) . "', `address_1` = '" . $this->db->escape($data['address_1']) . "', `phone` = '" . $this->db->escape($data['phone']) . "', `city` = '" . $this->db->escape($data['city']) . "', `postcode` = '" . $this->db->escape($data['postcode']) . "', `country_id` = '" . (int) $data['country_id'] . "', `zone_id` = '" . (int) $data['zone_id'] . "', `custom_field` = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode([])) . "', `default` = '" . (!empty($data['default']) ? (bool) $data['default'] : 0) . "'");
	...
	public function getTotalGuests(array $filter = []): int
	{
	...
	public function getGuests(array $filter = []): array
	{
		...
		if (!empty($filter['start']) || !empty($filter['limit'])) {
			$filter['start'] = isset($filter['start']) && (int) $filter['start'] >= 0 ? (int) $filter['start'] : 0;
			$filter['limit'] = isset($filter['limit']) && (int) $filter['limit'] > 0 ? (int) $filter['limit'] : 20;
