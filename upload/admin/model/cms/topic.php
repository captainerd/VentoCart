...
class Topic extends \Ventocart\System\Engine\Model
{
	...
	public function addTopic(array $data): int
	{
		...
		foreach ($data['topic_seo_url'] as $language_id => $keyword) {

			$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET   `language_id` = '" . (int) $language_id . "', `key` = 'topic_id', `value`= '" . (int) $topic_id . "', `keyword` = '" . $this->db->escape($keyword) . "'");
	...
	public function getSeoUrls(int $topic_id): array
	{
		...
		foreach ($query->rows as $result) {
			$topic_seo_url_data[$result['language_id']] = $result['keyword'];
		}
