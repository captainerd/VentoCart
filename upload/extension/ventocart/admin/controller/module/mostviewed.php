...
class MostViewed extends \Ventocart\System\Engine\Controller
{
	public function index(): void
	{
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = [
				...
				'href' => $this->url->link('extension/ventocart/module/mostviewed', 'user_token=' . $this->session->data['user_token'])
			];
		} else {
			$data['breadcrumbs'][] = [
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/ventocart/module/mostviewed', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'])
			];
		}

		if (!isset($this->request->get['module_id'])) {
			$data['save'] = $this->url->link('extension/ventocart/module/mostviewed.save', 'user_token=' . $this->session->data['user_token']);
		} else {
			$data['save'] = $this->url->link('extension/ventocart/module/mostviewed.save', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id']);
		}
	...
	public function save(): void
	{
		...
		if (!$this->user->hasPermission('modify', 'extension/ventocart/module/mostviewed')) {
		...
		if (!$json) {
			...
			if (!$this->request->post['module_id']) {
				$json['module_id'] = $this->model_setting_module->addModule('ventocart.mostviewed', $this->request->post);
			} else {
