 ...
 */
class CustomerActivity extends \Ventocart\System\Engine\Controller {
	/**
	 * @return void
	 */
	public function index(): void {
		$this->load->language('extension/ventocart/report/customer_activity');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/ventocart/report/customer_activity', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/ventocart/report/customer_activity.save', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/ventocart/report/customer_activity_form', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/ventocart/report/customer_activity')) {
	...
	public function report(): void {
		...
		$this->response->setOutput($this->load->view('extension/ventocart/report/customer_activity', $data));
	}
	...
	public function getReport(): string {
		...
		$activity_total = $this->model_extension_ventocart_report_customer->getTotalCustomerActivities($filter_data);

		$results = $this->model_extension_ventocart_report_customer->getCustomerActivities($filter_data);
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'url'   => $this->url->link('extension/ventocart/report/customer_activity.report', 'user_token=' . $this->session->data['user_token'] . '&code=customer_activity' . $url . '&page={page}')
		]);
		...
		return $this->load->view('extension/ventocart/report/customer_activity_list', $data);
	}
}
