        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-sm-3"><strong><?= $store ?></strong>
              ...
              <?php if ($geocode): ?>
                <a href="https://maps.google.com/maps?q=<?= urlencode($geocode) ?>&hl=<?= $geocode_hl ?>&t=m&z=15"
                  target="_blank" class="btn btn-info"><i class="fa-solid fa-location-dot"></i>
                  <?= $this->e($button_map) ?></a>
              <?php endif; ?>
            </div>
            <?php if ($telephone): ?>
          ...
          <?php foreach ($locations as $location): ?>
            <div class="card">
              <div class="card-header">
                <h4 class="card-title pt-2"><a href="#collapse-location-<?= $this->e($location['location_id']) ?>"
                    class="accordion-toggle" data-bs-toggle="collapse"
                    data-bs-parent="#accordion"><?= $this->e($location['name']) ?> <i
                      class="fa-solid fa-caret-down"></i></a></h4>
              </div>
              <div class="card-collapse collapse" id="collapse-location-<?= $this->e($location['location_id']) ?>">
                <div class="card-body">
                  <div class="row">
                    <?php if ($location . image): ?>
                      <div class="col-sm-3"><img src="<?= $this->e($location['image']) ?>"
                          alt="<?= $this->e($location['name']) ?>" title="<?= $this->e($location['name']) ?>"
                          class="img-thumbnail" /></div>
                    <?php endif; ?>
                    <div class="col-sm-3"><strong><?= $this->e($location['name']) ?></strong>
                      ...
                      <?php if ($location['geocode']): ?>
                        <a href="https://maps.google.com/maps?q=<?= urlencode($this->e($location['geocode'])) ?>&hl=<?= $this->e($geocode_hl) ?>&t=m&z=15"
                          target="_blank" class="btn btn-info"><i class="fa-solid fa-location-dot"></i>
                          <?= $this->e($button_map) ?></a>
                      <?php endif; ?>
                    </div>
          ...
          <div class="row mb-3 required">
            ...
            <div class="col-sm-10">
              <input type="text" name="<?= $email_field ?>" value="<?= $this->e($email) ?>" id="<?= $email_field ?>"
                class="form-control">
              <div id="error-<?= $email_field ?>" class="invalid-feedback"></div>
            </div>
          </div>
