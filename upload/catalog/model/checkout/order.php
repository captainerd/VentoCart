...
class Order extends \Ventocart\System\Engine\Model
{
	public function addOrder(array $data): int
	{
		...
		`email` = '" . $this->db->escape((string) $data['email']) . "',
	     ...
	     `telephone` = '" . $this->db->escape((string) (isset($data['telephone'])
						? $data['telephone']
						: ($this->customer->getTelephone() ? $this->customer->getTelephone() : ""))) . "',
		...
		`payment_firstname` = '" . $this->db->escape((string) $data['payment_firstname']) . "',
		`payment_lastname` = '" . $this->db->escape((string) $data['payment_lastname']) . "',
		`payment_company` = '" . $this->db->escape((string) $data['payment_company']) . "',
		`payment_address_1` = '" . $this->db->escape((string) $data['payment_address_1']) . "',
		`payment_phone` = '" . $this->db->escape((string) $data['payment_phone']) . "',
		`payment_city` = '" . $this->db->escape((string) $data['payment_city']) . "',
		`payment_postcode` = '" . $this->db->escape((string) $data['payment_postcode']) . "',
		`payment_country` = '" . $this->db->escape((string) $data['payment_country']) . "',
		`payment_country_id` = '" . (int) $data['payment_country_id'] . "',
		`payment_zone` = '" . $this->db->escape((string) $data['payment_zone']) . "',
		`payment_zone_id` = '" . (int) $data['payment_zone_id'] . "',
		`payment_address_format` = '" . $this->db->escape((string) $data['payment_address_format']) . "',
		...
		`shipping_firstname` = '" . $this->db->escape((string) $data['shipping_firstname']) . "',
		`shipping_lastname` = '" . $this->db->escape((string) $data['shipping_lastname']) . "',
		`shipping_company` = '" . $this->db->escape((string) $data['shipping_company']) . "',
		`shipping_address_1` = '" . $this->db->escape((string) $data['shipping_address_1']) . "',
		`shipping_phone` = '" . $this->db->escape((string) $data['shipping_phone']) . "',
		`shipping_city` = '" . $this->db->escape((string) $data['shipping_city']) . "',
		`shipping_postcode` = '" . $this->db->escape((string) $data['shipping_postcode']) . "',
		`shipping_country` = '" . $this->db->escape((string) $data['shipping_country']) . "',
		`shipping_country_id` = '" . (int) $data['shipping_country_id'] . "',
		`shipping_zone` = '" . $this->db->escape((string) $data['shipping_zone']) . "',
		`shipping_zone_id` = '" . (int) $data['shipping_zone_id'] . "',
		`shipping_address_format` = '" . $this->db->escape((string) $data['shipping_address_format']) . "',
		...
		if ($order_info) {
				...
				foreach ($order_products as $order_product) {
					...
					if ($order_product['type'] == 0) {
						...
						if ($order_product['variation_id']) {
							...
							$this->db->query("UPDATE `" . DB_PREFIX . "variations` SET `quantity` = (`quantity` - " . (int) $order_product['quantity'] . ") WHERE `variation_id` = '" . (int) $order_product['variation_id'] . "' AND `subtract` = '1'");
						}

						$order_options = $this->getOptions($order_id, $order_product['order_product_id']);

						foreach ($order_options as $order_option) {
							$this->db->query("UPDATE `" . DB_PREFIX . "product_options` SET `quantity` = (`quantity` - " . (int) $order_product['quantity'] . ") 
						WHERE `product_option_id` = '" . (int) $order_option['product_option_id'] . "' AND `subtract` = '1'");
						}
					}


					if ($order_product['type'] == 2) { // Check if the product type is a gift card
						$this->load->model('giftcards/giftcard');
						$this->model_giftcards_giftcard->processGiftCard($order_product);
					}
			...
			if (in_array($order_info['order_status_id'], array_merge((array) $this->config->get('config_processing_status'), (array) $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge((array) $this->config->get('config_processing_status'), (array) $this->config->get('config_complete_status')))) {
				...
				foreach ($order_products as $order_product) {
					...
					foreach ($order_options as $order_option) {
						$this->db->query("UPDATE `" . DB_PREFIX . "product_options` SET `quantity` = (`quantity` + " . (int) $order_product['quantity'] . ") WHERE `product_option_id` = '" . (int) $order_option['product_option_id'] . "' AND `subtract` = '1'");
					}
				}
