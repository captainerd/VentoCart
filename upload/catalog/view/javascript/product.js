$(document).ready(function () {
    ...
    $(".magnifyglass").blowup();
        //Gallery Splider (for when the thumbnails exceed the container)
     
        var scrollSpeed = 2; // Adjust the scroll speed as needed
        var leftscroll = 0;
        var leftLimit = 0; // Set the left limit
        var rightLimit = container.prop('scrollWidth') - container.width(); // Set the right limit
        var curDown = false;
        $(".slider-container").scrollLeft(0); // Initialize
        ...
        function scrolltoRight() {
            ...
            if (currentScroll + scrollSpeed < rightLimit) {
        ...
        function scrolltoLeft() {
            var currentScroll = container.scrollLeft();
            container.scrollLeft(Math.max(currentScroll - scrollSpeed, leftLimit));

            if (currentScroll - scrollSpeed > leftLimit) {
                leftscroll -= scrollSpeed;
            } else {
                clearInterval(intervalScroll);
            }
        ...
        $(".slider-container ul").on('mouseleave', stopScroll);

        $("#scrollleft").on('mousemove', function (e) {

            clearInterval(intervalScroll);
            intervalScroll = setInterval(scrolltoLeft, 10);
            wasLeft = true;
        });
        $("#scrollright").on('mousemove', function (e) {
            wasLeft = false;

            clearInterval(intervalScroll);
            intervalScroll = setInterval(scrolltoRight, 10);
        });

        prevXPos = 0;
        let wasLeft = false;
        $(".slider-container").mousemove(function (m) {
            if (curDown) {
                var scrollSpeed = Math.abs(prevXPos - m.pageX); // Adjust the multiplier as needed
                var container = $(".slider-container");
                var currentScroll = container.scrollLeft();

                $(".slider-container img").css('pointer-events', 'none');
                $(".slider-container a").css('pointer-events', 'none');

                $("#scrollleft").css('pointer-events', 'none');
                $("#scrollright").css('pointer-events', 'none');

                if (prevXPos < m.pageX) {
                    // Scroll to the left
                    wasLeft = true;
                    container.scrollLeft(Math.max(currentScroll - scrollSpeed, 0));
                } else if (prevXPos > m.pageX) {
                    // Scroll to the right
                    wasLeft = false;
                    container.scrollLeft(currentScroll + scrollSpeed);
                }
        ...
        $(".slider-container").mouseup(function () {
            if (wasLeft) $(".slider-container").animate({ scrollLeft: container.scrollLeft() - 80 }, 600);
            if (!wasLeft) $(".slider-container").animate({ scrollLeft: container.scrollLeft() + 80 }, 600);
            setTimeout(() => {
                $(".slider-container img").css('pointer-events', 'all');
                $(".slider-container a").css('pointer-events', 'all');

                $("#scrollleft").css('pointer-events', 'all');
                $("#scrollright").css('pointer-events', 'all');
            }, 600);
        ...
        function enableDisable(ignore, value) {
            ...
            let sets = new Set();
            $("#form-product div").each(function () {
                var divId = $(this).attr('id');

                // Ignore own options group set and irrelevant divs
                if (typeof divId === 'undefined' || divId === ignore) return true;

                // Loop through inputs or options within the current div
                $(this).find("input, select option").each(function () {
                    var inputId = $(this).attr('id');

                    $("#" + inputId).attr('disabled', false);
                    $("label[for='" + inputId + "'] img").css('opacity', '1');

                    sets.add(inputId.replace('input-option-value-', ''));
                    sets.add(value)
                    if (checkOptionSets(sets)) {
                        $("#" + inputId).attr('disabled', true);
                        $("label[for='" + inputId + "'] img").css('opacity', '0.4');
                    }
                    sets.clear();
        ...
        function checkOptionSets(Options) {
            //Check the quantity between variation option sets 
            return Object.keys(productVariation).some((key) => {
                let deset = new Set(Options);
                Object.keys(productVariation[key].options).some((keyo) => {
                    if (deset.has(productVariation[key].options[keyo].p_opt_value_id)) {
                        deset.delete(productVariation[key].options[keyo].p_opt_value_id);
                    }
                });
                if (deset.size === 0 && productVariation[key].quantity == 0) {
                    return true;
                }
            });
            return false;
