...
class Login extends \Ventocart\System\Engine\Controller
{
	...
	public function apilogin(): void
	{
		...
		// Verify API client and signed session
		if (!$this->customer->isApiClient() || !$this->customer->isApiSigned()) {
			$json['error']['warning'] = $this->language->get('error_invalid_api_access');
		} else {
			// Validate signature
			$expected_signature = hash('sha256', $this->request->post['password'] . $this->request->post['email'] . API_PUBLIC_KEY);
			if ($this->request->post['signature'] !== $expected_signature) {
				$json['error']['warning'] = $this->language->get('error_invalid_signature');
			} else {
				...
				// Check if customer is approved and perform login
				$customer_info = $this->model_account_customer->getCustomerByEmail($this->request->post['email']);
				...
				if ($customer_info && !$customer_info['status']) {
					...
					// Login successful, set session and response
					$this->session->data['customer'] = [
						'customer_id' => $customer_info['customer_id'],
						'customer_group_id' => $customer_info['customer_group_id'],
						'firstname' => $customer_info['firstname'],
