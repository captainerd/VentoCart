...
class Address extends \Ventocart\System\Engine\Controller
{
	...
	public function index(): array
	{
		...
		if (isset($this->session->data['payment_address']['address_id']) && $this->session->data['payment_address']['address_id'] == 0) {
			...
			foreach ($data['addresses'] as $address) {
				$tempPaymentAddress = $paymentAddress;
				unset($tempPaymentAddress['address_id']);
				$tempAddress = $address;
				unset($tempAddress['address_id']);
				if ($tempPaymentAddress == $tempAddress) {
					$matchedAddressId = $address['address_id'];
					break;
				}
			}
			if ($matchedAddressId > 0) {
				$this->session->data['payment_address']['address_id'] = $matchedAddressId;
			}
		} else {
	...
	public function save(): void
	{
		...
		if (!$json) {
			...
			$phonetest = str_replace(array(" ", "-", "(", ")"), "", $this->request->post['phone']);
			//check against regular expression, it starts with + followed by numbers only, minimum 6 chars, max 15
			if (!isset($this->request->post['phone']) || !preg_match('/^\+[0-9]{6,15}$/', $phonetest)) {
